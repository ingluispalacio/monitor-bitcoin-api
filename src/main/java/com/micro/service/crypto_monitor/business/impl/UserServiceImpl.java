...
public Mono<UserResponseDTO> create(UserRequestDTO dto) {
    System.out.println("🔵🔵🔵 CREATE SERVICE - Iniciando creación");
    ...
    return repository.findByUsername(dto.getUsername())
            .flatMap(user -> {
                System.err.println("❌ Username ya existe: " + user.getUsername());
                return Mono.error(new IllegalArgumentException("Username already exists"));
            })
            .switchIfEmpty(
                repository.findByEmail(dto.getEmail())
                    .flatMap(user -> {
                        System.err.println("❌ Email ya existe: " + user.getEmail());
            ...
            .then(
                Mono.defer(() -> {
                    try {
                        ...
                        System.out.println("✅ Usuario construido con ID: " + newUser.getId());
                        return repository.save(newUser);
                    } catch (Exception e) {
                        System.err.println("❌ Error construyendo usuario: " + e.getMessage());
                        e.printStackTrace();
                        return Mono.error(e);
                    }
                })
            ...
            .map(user -> {
                System.out.println("✅ Usuario guardado en BD: " + user.getId());
                return mapToDTO(user);
            })
            .doOnError(error -> {
                System.err.println("❌ Error en create(): " + error.getMessage());
